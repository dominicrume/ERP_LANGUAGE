...
Both cover the locales this release ships. A parked country's numbers are
in git history and come back with the file.
...
def test_the_legacy_unweighted_points_form_still_scores_exactly_as_it_did():
    assert len(LEGACY_GOLDEN) == 10   # 2 shipped locales x 5 options
...
def test_the_shipped_templates_match_the_v1_weighted_golden():
    assert len(WEIGHTED_GOLDEN) == 20  # 2 templates x 2 locales x 5 options
